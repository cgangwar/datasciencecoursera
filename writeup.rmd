...
An overall pseudo-random number generator seed was set at 123456 for all code. In order to reproduce the results below, the same seed should be used.
...
trainingset <- read.csv("C:/dev/coursera/practicalml/pml-training.csv", na.strings=c("NA","#DIV/0!", ""))

# Loading the testing data set 
testingset <- read.csv("C:/dev/coursera/practicalml/pml-testing.csv", na.strings=c("NA","#DIV/0!", ""))
...
```{r} 
plot(subTraining$classe, col="blue", main="levels of classe within subTraining data set", xlab="classe levels", ylab="Frequency")
```
...
Accuracy for Random Forest model was 0.996 (95% CI: (0.993, 0.997)) compared to 0.739 (95% CI: (0.719, 0.744)) for Decision Tree model. The random Forest model is choosen. The accuracy of the model is 0.996. The expected out-of-sample error is estimated at 0.005, or 0.5%. The expected out-of-sample error is calculated as 1 - accuracy for predictions made against the cross-validation set. Our Test data set comprises 20 cases. With an accuracy above 99% on our cross-validation data, we can expect that very few, or none, of the test samples will be missclassified.
